# Autogenerated wrapper script for GStreamer_jll for aarch64-apple-darwin
export gst_inspect, gst_launch, gst_stats, gst_tester, gst_typefind, libgstbase, libgstcheck, libgstcontroller, libgstnet, libgstreamer

using Glib_jll
using GMP_jll
using GSL_jll
JLLWrappers.@generate_wrapper_header("GStreamer")
JLLWrappers.@declare_library_product(libgstbase, "@rpath/libgstbase-1.0.0.dylib")
JLLWrappers.@declare_library_product(libgstcheck, "@rpath/libgstcheck-1.0.0.dylib")
JLLWrappers.@declare_library_product(libgstcontroller, "@rpath/libgstcontroller-1.0.0.dylib")
JLLWrappers.@declare_library_product(libgstnet, "@rpath/libgstnet-1.0.0.dylib")
JLLWrappers.@declare_library_product(libgstreamer, "@rpath/libgstreamer-1.0.0.dylib")
JLLWrappers.@declare_executable_product(gst_inspect)
JLLWrappers.@declare_executable_product(gst_launch)
JLLWrappers.@declare_executable_product(gst_stats)
JLLWrappers.@declare_executable_product(gst_tester)
JLLWrappers.@declare_executable_product(gst_typefind)
function __init__()
    JLLWrappers.@generate_init_header(Glib_jll, GMP_jll, GSL_jll)
    JLLWrappers.@init_library_product(
        libgstbase,
        "lib/libgstbase-1.0.0.dylib",
        RTLD_LAZY | RTLD_DEEPBIND,
    )

    JLLWrappers.@init_library_product(
        libgstcheck,
        "lib/libgstcheck-1.0.0.dylib",
        RTLD_LAZY | RTLD_DEEPBIND,
    )

    JLLWrappers.@init_library_product(
        libgstcontroller,
        "lib/libgstcontroller-1.0.0.dylib",
        RTLD_LAZY | RTLD_DEEPBIND,
    )

    JLLWrappers.@init_library_product(
        libgstnet,
        "lib/libgstnet-1.0.0.dylib",
        RTLD_LAZY | RTLD_DEEPBIND,
    )

    JLLWrappers.@init_library_product(
        libgstreamer,
        "lib/libgstreamer-1.0.0.dylib",
        RTLD_LAZY | RTLD_DEEPBIND,
    )

    JLLWrappers.@init_executable_product(
        gst_inspect,
        "bin/gst-inspect-1.0",
    )

    JLLWrappers.@init_executable_product(
        gst_launch,
        "bin/gst-launch-1.0",
    )

    JLLWrappers.@init_executable_product(
        gst_stats,
        "bin/gst-stats-1.0",
    )

    JLLWrappers.@init_executable_product(
        gst_tester,
        "bin/gst-tester-1.0",
    )

    JLLWrappers.@init_executable_product(
        gst_typefind,
        "bin/gst-typefind-1.0",
    )

    JLLWrappers.@generate_init_footer()
end  # __init__()
